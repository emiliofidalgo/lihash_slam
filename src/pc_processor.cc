...
PointCloudProcessor::PointCloudProcessor(const ros::NodeHandle& nh) :
  nh_(nh),
  ncores_(2) {
...
void PointCloudProcessor::readParams() {
  // Reading parameters
...
void PointCloudProcessor::process(const PointCloud::Ptr& pc_in, PointCloud::Ptr& pc_out) {
  
  // Split the pointcloud into scan_lines_ scans
  std::vector<PointCloud::Ptr> scans;
  splitPointCloud(pc_in, scans);

  // Extracting features from each scan
  extractFeatures(scans, pc_out);   
}

bool PointCloudProcessor::isValidPoint(const double x, const double y, const double z, double* dist) {
...
void PointCloudProcessor::splitPointCloud(const PointCloud::Ptr& pc_in, std::vector<PointCloud::Ptr>& scans) {
...
void PointCloudProcessor::extractFeatures(const std::vector<PointCloud::Ptr>& scans, PointCloud::Ptr& pc_edges) {
  ...
  for (int i = 0; i < scan_lines_; i++) {
    ...
    // Extract edges from each region
...
void PointCloudProcessor::extractFeaturesFromRegion(const PointCloud::Ptr& pc_in, std::vector<SmoothnessItem>& smooths, PointCloud::Ptr& pc_edges) {
