...
struct Loop {
  explicit Loop(LoopFrame* f1, LoopFrame* f2, const Eigen::Isometry3d& pose) : 
    frame1(f1),
    frame2(f2),
    rel_pose(pose) {
  }

  explicit Loop(LoopFrame* f1, LoopFrame* f2) : 
    frame1(f1),
    frame2(f2),
  ...
  LoopFrame* frame1;
  LoopFrame* frame2;
  Eigen::Isometry3d rel_pose; // Estimated relative pose
};
...
class LoopDetector {
 public:
  ...
  virtual void init() = 0;
  virtual void addFrame(const Eigen::Isometry3d& pose, const PointCloud::Ptr& points) = 0;
  virtual void detect(const Eigen::Isometry3d& pose, const PointCloud::Ptr& points, std::vector<Loop>& loops) = 0;
};
